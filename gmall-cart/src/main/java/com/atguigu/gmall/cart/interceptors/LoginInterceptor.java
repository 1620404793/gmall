...
import com.atguigu.gmall.cart.config.JwtPropreties;
...
public class LoginInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        String userKey = CookieUtils.getCookieValue(request, jwtPropreties.getUserKey());
        //判断有没有userKey，没有：制作一个放入cookie中
        if (StringUtils.isEmpty(userKey)){
            userKey = UUID.randomUUID().toString();
            CookieUtils.setCookie(request,response,jwtPropreties.getUserKey(),userKey,6*30*24*3600);
        }
